import { combineLatest, fromEvent, map, Observable, pluck, startWith, tap, withLatestFrom } from "rxjs";
...
const sliderSeqiemce$ = combineLatest([
  quality$,
  rating$,
  actual$,
])
...
fromEvent<MouseEvent>(
  document.querySelector('#send-result') as HTMLButtonElement, 'click',
)
  .pipe(
    withLatestFrom(sliderSeqiemce$)
  )
  .subscribe(([_e, value]) => {
    console.log(value);
  })
